...
class UnetPlus(nn.Module):
    def __init__(self, args):
        ...
        if args.in_channels == 3:
            ...
            self.firstconv = nn.Conv2d(args.in_channels, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)
        ...
        self.logit1 = nn.Conv2d(64, args.num_classes, kernel_size=1)
        self.logit2 = nn.Conv2d(64, args.num_classes, kernel_size=1)
        self.logit3 = nn.Conv2d(128, args.num_classes, kernel_size=1)
        self.logit4 = nn.Conv2d(256, args.num_classes, kernel_size=1)
        self.logit_conf = nn.Conv2d(256, args.num_classes, kernel_size=1)
        self.conf_repeat = args.conf_repeat
    ...
    def forward(self, x, conf=False):
        ...
        logit4 = self.logit4(x0_4) # b, 256, h/2, w/2 -> b, c, h/2, w/2
        logit = self.mix[1] * logit1 + self.mix[2] * logit2 + self.mix[3] * logit3 + self.mix[4] * logit4
        logit = F.interpolate(logit, size=(H, W), mode='bilinear', align_corners=False)
        if conf:
            logit_conf = self.logit_conf(x0_4)
            logit_conf = F.interpolate(logit_conf, size=(H, W), mode='bilinear', align_corners=False)
            logit_total = torch.zeros((self.conf_repeat, *logit.size())).to(logit_conf.device)
            for t in range(self.conf_repeat):
                epsilon = torch.randn(logit_conf.size()).to(logit_conf.device)
                logit_total[t] = logit + torch.mul(torch.mul(logit_conf, epsilon), torch.sigmoid(logit_conf))
            return logit_total
        return logit
