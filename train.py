...
def parse_args():
    ...
    parser.add_argument('--conf_repeat', type=int, default=4, help='number of repeated variational samplings')
...
def train_BigBrain(epoch):
    ...
    for i, data in enumerate(train_loader):
        ...
        for t in range(output_2.shape[0]):
            loss += criterion(output_2[t], target_2)
        optimizer.zero_grad()
        loss.backward()
        if i%50 == 0:
...
def train_AllenDataset(epoch):
    ...
    for i, data in enumerate(train_loader):
        ...
        loss.backward()
        if i % 50 == 0:
            print(f'Taining {i} of epoch {epoch}|loss {loss.item():.4f}')


if __name__ == '__main__':
    args = parse_args()
    _train = eval(f'train_{args.training}')
    if args.training == 'AllenDataset':
        train_loader = get_data_loader(args, choice=1)
    else:
        train_loader = get_data_loader(args, choice=2)
